...
export class PatientListComponent extends BaseListComponent<PatientListDto> implements OnInit, OnDestroy {
  ...
  modifiedOnDateField: string = 'modifiedOn';
  selectedModifiedOnDate?: string;
  ...
  override setFilterValuesToModel(sourceInfo: SourceInfo) {
    ...
    let isExistDateFields = sourceInfo.filters.filter(e => e.filterName === this.modifiedOnDateField);
    if (isExistDateFields?.length) {
      let toBeBindedInDateInput = isExistDateFields.find(e => e.operator === ConstString.GeSplit);
      if (toBeBindedInDateInput) {
        this.selectedModifiedOnDate = toBeBindedInDateInput.filterVal;
      }
    }
  }
  ...
  onModifiedDateChange() {
    let toBeAppliedDateFilterModel: FilterModel[] = [];
    let nextSelectedModifiedOnDate: string = '';
    if (this.selectedModifiedOnDate) {
      const modifiedOnDate = new Date(this.selectedModifiedOnDate + 'T00:00:00Z');
      modifiedOnDate.setUTCDate(modifiedOnDate.getUTCDate() + 1);
      nextSelectedModifiedOnDate = modifiedOnDate.toISOString().substring(0, 10);
    }
    toBeAppliedDateFilterModel.push({ filterName: this.modifiedOnDateField, filterVal: this.selectedModifiedOnDate || '', operator: ConstString.GeSplit, splitOp: ConstString.AndSplit });
    toBeAppliedDateFilterModel.push({ filterName: this.modifiedOnDateField, filterVal: nextSelectedModifiedOnDate || '', operator: ConstString.LtSplit, splitOp: '' });
    super.onFilterMultipleFields(toBeAppliedDateFilterModel);
  }
  ...
  override onClearFilter() {
    ...
    this.selectedModifiedOnDate = undefined;
    super.onClearFilter();
  }
